...
lm_set <- fread(file.path(data.dir,'/source_data/LM22.txt'))
...
if(pull.rna){
  ...
  write.table( joined_file,  file = file.path(data.dir,"/results/rna.htseq.all.txt"), row.names = FALSE, quote = FALSE, sep = "\t")
  write.table( all_rna_raw,  file = file.path(data.dir,"/results/rna.tpm.all.txt"), row.names = FALSE, quote = FALSE, sep = "\t")

} else {

  joined_file <- fread(file.path(data.dir,"/results/rna.htseq.all.txt"))
  all_rna_raw <- fread(file.path(data.dir,"/results/rna.tpm.all.txt"))

}

#### remove FFPE ####

key_tumour_features <- fread(file.path(data.dir,'/source_data/BTC_clinical.tumours.tsv'))
...
if(overwrite.mn.cohort){
  ...
  good_sample_list <- fread(file.path(data.dir,'/source_data/sample.list.txt'), header=F)
  ...
  write.table( tpm.good,  file = file.path(data.dir,"/results/rna.tpm.ff.txt"), row.names = FALSE, quote = FALSE, sep = "\t")
  write.table( htseq.good,  file = file.path(data.dir,"/results/rna.read.ff.txt"), row.names = FALSE, quote = FALSE, sep = "\t")
